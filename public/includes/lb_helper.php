...
class LicenseBoxAPI{
	...
	public function __construct(){ 
		$this->product_id = '7B61507A';
		$this->api_url = 'http://licensebox.Azha.com/';
		$this->api_key = 'D387CEED9766B0117BB6';
		$this->api_language = 'english';
		$this->current_version = 'v3.6.7';
	...
	public function get_subscriptions(){
		...
		$get_data = $this->call_api(
			'POST',
			$this->api_url.'api/get_subscriptions', 
			json_encode($data_array)
		);
		$response = json_decode($get_data, true);
		return $response;
	...
	public function post_subscriptions($data,$business){
		$machine_id=$this->UniqueMachineID();
		$data_array =  array(
			'machine_id'=>$machine_id,
			'data'=>$data,
		...
		$get_data = $this->call_api(
			'POST',
			$this->api_url.'api/post_subscriptions', 
			json_encode($data_array)
		);
		$response = json_decode($get_data, true);
		return $response;
	...
	public function UniqueMachineID($salt = "") {
		...
		return $result; ;//md5($salt.md5($result));
	}
	...
	public function verify_license($time_based_check = false,$license = false, $client = false,$local=true){
		//return $machine_id=$this->UniqueMachineID();
		if($local){
			$machine_id=$this->UniqueMachineID();
			$data_array =  array(
				"product_id"  => $this->product_id,
