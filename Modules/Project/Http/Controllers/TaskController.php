            ...
            if (request()->get('task_view') == 'list_view') {
                return Datatables::of($project_task)
                    ->addColumn('action', function ($row) use ($can_crud) {
                        $html = '<div class="btn-group">
                                    <button class="btn btn-info dropdown-toggle btn-xs" type="button"  data-toggle="dropdown" aria-expanded="false">
                                      ...
                                      <ul class="dropdown-menu dropdown-menu-left" role="menu">
                                       <li>
                                            <a data-href="' . action('\Modules\Project\Http\Controllers\TaskController@show', [$row->id, 'project_id' => $row->project_id]) . '" class="cursor-pointer view_a_project_task">
                                                <i class="fa fa-eye"></i>
                                                '.__("messages.view").'
                                            </a>
                                        </li>
                        ...
                        if ($can_crud) {
                            $html .= '<li>
                                    <a data-href="' . action('\Modules\Project\Http\Controllers\TaskController@edit', [$row->id, 'project_id' => $row->project_id]) . '" class="cursor-pointer edit_a_project_task">
                                        <i class="fa fa-edit"></i>
                                        '.__("messages.edit").'
                                    </a>
                                </li>
                                <li>
                                    <a data-href="' . action('\Modules\Project\Http\Controllers\TaskController@destroy', [$row->id, 'project_id' => $row->project_id]) . '" class="cursor-pointer delete_a_project_task">
                    ...
                    ->editColumn('subject', '
                            <a data-href="{{action("\Modules\Project\Http\Controllers\TaskController@show", [$id, "project_id" => $project_id])}}" class="cursor-pointer view_a_project_task text-black">
                ...
                foreach ($project_tasks as $key => $tasks) {
                    ...
                    foreach ($tasks as $task) {
                        ...
                        $delete = '';
                        if ($can_crud) {
                            $edit = action('\Modules\Project\Http\Controllers\TaskController@edit', [$task->id, 'project_id' => $task->project_id]);

                            $delete = action('\Modules\Project\Http\Controllers\TaskController@destroy', [$task->id, 'project_id' => $task->project_id]);
                        }

                        $view = action('\Modules\Project\Http\Controllers\TaskController@show', [ $task->id, 'project_id' => $task->project_id]);
    ...
    public function store(Request $request)
    {
        try {
            ...
            if (!empty($task_members['attached'])) {
                ...
                $project_task['link'] = action('\Modules\Project\Http\Controllers\ProjectController@show', [$project_task->project_id]);
